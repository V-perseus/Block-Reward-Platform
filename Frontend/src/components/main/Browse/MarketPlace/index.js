...
export const MarketPlace = (props) => {
  ...
  console.log("url",url)
  console.log(url.includes("CBD_plus_reedme"))
  console.log(url.includes("CBD_plus_membership"))

  return(
    <Container>
      {
        url.includes("CBD_plus_reedme") && (
          <MarketListWrapper>   
      ...
      {
        url.includes("CBD_plus_membership") && (
          <> <ContentHeader>
      ...
      }
      {
        url.includes("browse")&&(
          <>
           <ImageWrapper src = {CBD_image} onClick = {()=>{navigate('/creator/CBD_plus_membership')}} />
           <ContentHeader>
          <h2>{isCreator ? 'Browse' : (isCollection ? 'Browse BlockReward Originals' : 'Membership Market')}</h2>
          <FilterGroup>
            <Select
